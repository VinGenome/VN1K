...
# Note: Ensure the path to the original bam files is correctly relative or provided via environment
for file in "${DATA_DIR}"/*.ccs.bam; do
    echo "Processing $(basename "$file")"
    samtools bam2fq -@ "$THREADS" "$file" | gzip > "${ASM_DIR}/$(basename "${file%.ccs.bam}.fastq.gz")"
done
...
# Ensure hifiasm path is correctly referenced
