...
# Configuration
DATA_DIR=${VN1K_DATA:-"/QRISdata/Q3570/Data/sequencing_data/Trung/4_samples"}
THREADS=32
...
process_sample() {
    ...
    local combined_reads="${sample_dir}/${sample_id}_ONT_combined.fastq.gz"
    local flye_output_dir="${sample_dir}/flye_${sample_id}"
    local quast_dir="${sample_dir}/quast_ONT_${sample_id}"
    ...
    # Step 0: Concatenate and compress
    if [ "${sample_id}" == "HG01596" ]; then
        zcat 2022*_21-lee-006*.fastq.gz | gzip > "${combined_reads}"
    elif [ "${sample_id}" == "HG02018" ]; then
        zcat 2022*_${sample_id}_*.fastq.gz 2023*_${sample_id}_*.fastq.gz | gzip > "${combined_reads}"
    else
        zcat ${sample_id}_*.fastq.gz | gzip > "${combined_reads}"
    fi
    ...
    python quast.py "${flye_output_dir}/assembly.fasta" -o "${quast_dir}"
