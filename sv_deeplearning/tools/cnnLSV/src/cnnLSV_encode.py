        ...
        for k, v in read_dic.items():
            ...
            for i in range(len(v)-1):
                if (v[i+1][0]-v[i][1]) - (v[i+1][2]-v[i][3]) >= 50:
                    ...
                    ins_list.append(np.array([v[i][3], v[i][3]+ins_len], dtype=np.uint8))
                elif (v[i+1][2]-v[i][3]) - (v[i+1][0]-v[i][1]) >= 50:
                    del_list.append(np.array([v[i][3], v[i+1][2]], dtype=np.uint8))
            ...
            if e-s <= e2 - s1:
                ...
                if bp1 <= (s+e)//2 <= bp2:
                    ...
                    if e-s <= 2*(bp2 - bp1):
                        value += 4
                        read_base += e - s + 1
            color_list.append(np.array([s, e, value], dtype=np.uint8))
...
def generate_img(elelist, col, depth):
    imageback = torch.zeros((len(elelist), col), dtype=torch.uint8)
    index = 0
    for read in elelist:
        start, end, value = read
        imageback[index][start: end] = value
        index += 1
    imageback = torch.sort(imageback, dim=0)[::-1]
    imageback = imageback[0:depth, :]  # depth = 是计算出来的
