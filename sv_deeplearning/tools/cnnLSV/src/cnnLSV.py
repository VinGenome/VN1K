...
def parseArgs(argv):
    ...
    parser.add_argument('--callers', type=str, nargs='+', default=['cuteSV', 'svim', 'Sniffles2', 'pbsv', 'sniffles'])
    # others
...
def filter_sv(bam_fp, tempdir, task_sv, length, width, minmapq):
    ...
    for sv in sv_list:
        ...
        if svlen > maxlen or svlen < minlen:
            ...
            try:
                ...
                if pre:
                    true_list.append(index)
                del img, pre, draw_list
                gc.collect()
                # print("hiiiiiiiiii")
            except:
                # print("TASK except=", number)
                # print(error)
                true_list.append(index)
    ...
    f.close()
    bam.close()
    del true_list, bam, sv_list, index
    gc.collect()
...
def main_ctrl(args):
    ...
    mean_std = eval(open('./tools/cnnLSV/src/mean_std.txt').read())
